...
class CKA:
    ...
    def transformer_padding(self, x, window_size=16):
        mod_pad_h = 0
        mod_pad_w = 0
        _, _, h, w = x.size()
        if h % window_size != 0:
    ...
    def compare(self,
        ...
        for x1, x2 in tqdm(zip(dataloader1, dataloader2), desc="| Comparing features |", total=num_batches):
            ...
            x1 = x1['lq']
            x2 = x2['lq']
            x1 = self.transformer_padding(x1)
            x2 = self.transformer_padding(x2)
            # print(x1.shape, x2.shape)

            _ = self.model1(x1.to(self.device))
            _ = self.model2(x2.to(self.device))

            for i, (name1, feat1) in tqdm(enumerate(self.model1_features.items())):
                # print(feat1.shape)
                # X = feat1.flatten(1)
                ...
                for j, (name2, feat2) in enumerate(self.model2_features.items()):
                    ...
                    if i==j:
                        ...
                        print("2--", self.hsic_matrix[i, j, 1])
                        self.hsic_matrix[i, j, 2] += self._HSIC(L, L) / num_batches
                        print("3--", self.hsic_matrix[i, j, 2])

                        assert not torch.isnan(self.hsic_matrix).any(), "IJ computation resulted in NANs"
