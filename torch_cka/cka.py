...
class CKA:
    def __init__(self,
        ...
        if len(list(model1.modules())) > 150 and model1_layers is None:
            ...
            # print("Model 1 layers :", len(list(model1.modules())))
        ...
        if len(list(model2.modules())) > 150 and model2_layers is None:
            ...
            # print("Model 2 layers :", len(list(model2.modules())))
    ...
    def compare(self,
        ...
        for (x1, *_), (x2, *_) in tqdm(zip(dataloader1, dataloader2), desc="| Comparing features |", total=num_batches):
            ...
            self.model1_features = {}
            self.model2_features = {}
            _ = self.model1(x1.to(self.device), 2)
            _ = self.model2(x2.to(self.device), 3)

            for i, (name1, feat1) in enumerate(self.model1_features.items()):
                # print("feat1--", feat1.shape, feat1)
                X = feat1.flatten(1).reshape([B, -1])
                K = X @ X.t()
                ...
                for j, (name2, feat2) in enumerate(self.model2_features.items()):
                    # print("feat2", feat2.shape, feat2)
                    # if abs(i-j)<26:
                    if 1:
                        Y = feat2.flatten(1).reshape([B, -1])
                        L = Y @ Y.t()
                        L.fill_diagonal_(0)
                        # print("2--", L.shape, L)
        ...
        # loc = torch.where(self.hsic_matrix != self.hsic_matrix)
        # self.hsic_matrix[loc] = 0

        assert not torch.isnan(self.hsic_matrix).any(), "HSIC computation resulted in NANs"
        print("MAX and MIN -- ", self.hsic_matrix.max(), self.hsic_matrix.min(), self.hsic_matrix.mean())
        diag = torch.diag(self.hsic_matrix)
        print("DIG MAX and MIN -- ", diag.shape, diag.max(), diag.min(), diag.mean())
